...
В ходе данной лабораторной работы были освоены базовые принципы программирования на языке JavaScript и проведено сравнение с языком C#.

**Ключевые темы:**
1. **Настройка окружения:** Создание проекта, подключение JS к HTML, работа с Git и GitHub.
2. **Переменные и типы данных:** Изучение `let`, `const`, динамической типизации и оператора `typeof`.
3. **Преобразование типов:** Различия между явным и неявным приведением типов (coercion).
4. **Сравнение:** Понимание разницы между нестрогим (`==`) и строгим (`===`) сравнением.
5. **Условные операторы:** Использование конструкций `if/else`, логических операторов и `switch/case`.
6. **Запуск кода:** Выполнение скриптов как в браузере (Console), так и в среде Node.js.
